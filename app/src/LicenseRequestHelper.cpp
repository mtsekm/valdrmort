...
bool LicenseRequestHelper::sendPostRequest(const std::string &url, const std::string &drm_message,
    ...
    if (!curl) {
        logError("LicenseRequestHelper: Failed to initialize cURL.");
        return false;
    }
    ...
    if (!response_json) {
        logError("LicenseRequestHelper: Failed to parse JSON response.");
        curl_slist_free_all(headers);
        curl_easy_cleanup(curl);
        free(json_payload);
        return false;
...
void LicenseRequestHelper::logCurlError(const std::string &operation, int error_code) {
    logError("LicenseRequestHelper: " + operation +
             " failed with cURL error: " + curl_easy_strerror(static_cast<CURLcode>(error_code)));
}
